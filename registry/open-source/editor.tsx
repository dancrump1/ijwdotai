	...
	const defaultExtensions = [
		StarterKit.configure({
			...
			code: {
				HTMLAttributes: {
					class: cn(
						"rounded-md bg-muted px-1.5 py-1 font-medium font-mono"
					),
					spellcheck: "false",
				},
			},
		...
		CodeBlockLowlight.configure({
			...
			HTMLAttributes: {
				class: cn(
					...
					"[&_.hljs-doctag]:text-foreground [&_.hljs-keyword]:text-foreground [&_.hljs-meta_.hljs-keyword]:text-foreground [&_.hljs-template-tag]:text-foreground [&_.hljs-template-variable]:text-foreground [&_.hljs-type]:text-foreground [&_.hljs-variable.language_]:text-foreground",
					"[&_.hljs-title.class_.inherited__]:text-foreground [&_.hljs-title.class_]:text-foreground [&_.hljs-title.function_]:text-foreground [&_.hljs-title]:text-foreground",
					"[&_.hljs-attr]:text-foreground [&_.hljs-attribute]:text-foreground [&_.hljs-literal]:text-foreground [&_.hljs-meta]:text-foreground [&_.hljs-number]:text-foreground [&_.hljs-operator]:text-foreground [&_.hljs-selector-attr]:text-foreground [&_.hljs-selector-class]:text-foreground [&_.hljs-selector-id]:text-foreground [&_.hljs-variable]:text-foreground",
					"[&_.hljs-meta_.hljs-string]:text-foreground [&_.hljs-regexp]:text-foreground [&_.hljs-string]:text-foreground",
					"[&_.hljs-built_in]:text-foreground [&_.hljs-symbol]:text-foreground",
					"[&_.hljs-code]:text-foreground [&_.hljs-comment]:text-foreground [&_.hljs-formula]:text-foreground",
					"[&_.hljs-name]:text-foreground [&_.hljs-quote]:text-foreground [&_.hljs-selector-pseudo]:text-foreground [&_.hljs-selector-tag]:text-foreground",
					"[&_.hljs-subst]:text-foreground",
					"[&_.hljs-section]:font-bold [&_.hljs-section]:text-foreground",
					"[&_.hljs-bullet]:text-foreground",
					"[&_.hljs-emphasis]:text-foreground [&_.hljs-emphasis]:italic",
					"[&_.hljs-strong]:font-bold [&_.hljs-strong]:text-foreground",
					"[&_.hljs-addition]:bg-background [&_.hljs-addition]:text-foreground",
					"[&_.hljs-deletion]:bg-background [&_.hljs-deletion]:text-foreground"
				),
			},
		}),
		Superscript,
				...
				render: () => {
					...
					return {
						...
						onKeyDown(onKeyDownProps) {
							...
							return (
								handleCommandNavigation(onKeyDownProps.event) ?? false
							);
						},
						onExit() {
							popup.destroy();
	...
	>
		{children && Array.isArray(children)
			? children.reduce((acc: ReactNode[], child, index) => {
					...
					acc.push(<hr key={index + "editor"} orientation="vertical" />);
					acc.push(child);
					return acc;
				}, [])
			: children}
...
export const EditorLinkSelector = ({
	...
	const defaultValue = (editor.getAttributes("link") as { href?: string })
		.href;
	return (
		<Popover modal onOpenChange={onOpenChange} open={open}>
			<PopoverTrigger asChild>
