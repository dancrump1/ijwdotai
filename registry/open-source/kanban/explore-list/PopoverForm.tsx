		...
		<AnimatePresence>
			{isOpen && (
				<motion.div
					...
					layoutId={`popover-${uniqueId}`}
					className={cn(
						"absolute h-auto overflow-hidden border border-zinc-950/10 bg-white outline-none dark:bg-zinc-700 z-50", // Changed z-90 to z-50
						className
					)}
					style={{
						borderRadius: 12,
...
export function PopoverLabel({ children, className }: PopoverLabelProps) {
	...
	return (
		<motion.span
			...
			className={cn(
				"absolute pointer-events-none content-center left-4 top-0 bottom-0 select-none text-sm text-zinc-500 dark:text-zinc-400",
				className
			)}
		>
			{children}
...
export function PopoverTextarea({ className }: PopoverTextareaProps) {
	...
	return (
		<textarea
			className={cn(
				"h-full w-28 resize-none content-center rounded-md bg-transparent px-4 py-3 text-sm outline-none",
				className
			)}
			autoFocus
			value={note}
